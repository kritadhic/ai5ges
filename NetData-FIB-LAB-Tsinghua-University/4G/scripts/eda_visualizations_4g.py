"""
Phase 1: Exploratory Data Analysis and Visualization - FIB LAB 4G Dataset
==========================================================================
This script generates comprehensive visualizations for the preprocessed 4G dataset.
"""
...
print("="*80)
print("FIB LAB 4G NETWORK - EXPLORATORY DATA ANALYSIS")
print("="*80)

# Load processed data (sample for memory efficiency)
print("\nLoading processed data...")
print("Note: Loading a sample of data for visualization (every 10th row)...")
df = pd.read_csv('processed_data/fiblab_4g_all_processed.csv', skiprows=lambda i: i % 10 != 0 and i != 0)
df['DateTime'] = pd.to_datetime(df['DateTime'])
print(f"Loaded {len(df)} sampled records")
print(f"Original dataset: ~4.6M records")

# =============================================================================
# 1. TIME SERIES PLOTS
# =============================================================================
print("\n[1/10] Creating time series plots...")
...
axes[0].set_title('Average Total Energy Consumption Over Time (4G)', fontsize=14, fontweight='bold')
...
axes[1].set_title('Average PRB Usage Ratio Over Time (4G)', fontsize=14, fontweight='bold')
...
axes[2].set_title('Average Number of Users Over Time (4G)', fontsize=14, fontweight='bold')
...
# =============================================================================
print("\n[2/10] Creating weekday vs weekend comparison...")

fig, axes = plt.subplots(2, 2, figsize=(14, 10))

# Energy: Weekday vs Weekend
...
# 3. HOURLY PATTERNS
# =============================================================================
print("\n[3/10] Creating hourly pattern analysis...")
...
# =============================================================================
print("\n[4/10] Creating distribution plots...")
...
# =============================================================================
print("\n[5/10] Creating correlation heatmap...")
...
ax.set_title('Feature Correlation Heatmap (4G Network)', fontsize=14, fontweight='bold')
plt.tight_layout()
...
# =============================================================================
print("\n[6/10] Creating energy component analysis...")
...
axes[0].set_title('BBU Energy vs RRU Energy', fontsize=12, fontweight='bold')
...
axes[1].set_title('Average Energy Component Breakdown', fontsize=12, fontweight='bold')
...
# =============================================================================
print("\n[7/10] Creating efficiency metrics analysis...")
...
plt.tight_layout()
plt.savefig(f'{PLOTS_DIR}/07_efficiency_metrics.png', dpi=300, bbox_inches='tight')
plt.close()
print("  ✓ Saved: 07_efficiency_metrics.png")

# =============================================================================
# 8. LOAD VS ENERGY ANALYSIS
# =============================================================================
print("\n[8/10] Creating load vs energy analysis...")

fig, axes = plt.subplots(1, 2, figsize=(14, 5))

# PRB usage vs Energy
sample = df.sample(min(10000, len(df)))
scatter = axes[0].scatter(sample['PRB_usage_ratio'], sample['Total_energy'],
                         c=sample['Num_users'], cmap='viridis', alpha=0.5, s=20)
axes[0].set_title('PRB Usage vs Total Energy (colored by # users)', fontsize=12, fontweight='bold')
axes[0].set_xlabel('PRB Usage (%)')
axes[0].set_ylabel('Total Energy (W)')
axes[0].grid(True, alpha=0.3)
plt.colorbar(scatter, ax=axes[0], label='Number of Users')

# Traffic vs Energy
scatter2 = axes[1].scatter(sample['Traffic_volume'], sample['Total_energy'],
                          c=sample['PRB_usage_ratio'], cmap='plasma', alpha=0.5, s=20)
axes[1].set_title('Traffic Volume vs Total Energy (colored by PRB usage)', fontsize=12, fontweight='bold')
axes[1].set_xlabel('Traffic Volume (KByte)')
axes[1].set_ylabel('Total Energy (W)')
axes[1].set_xlim(0, df['Traffic_volume'].quantile(0.95))
axes[1].grid(True, alpha=0.3)
plt.colorbar(scatter2, ax=axes[1], label='PRB Usage (%)')

plt.tight_layout()
plt.savefig(f'{PLOTS_DIR}/08_load_vs_energy.png', dpi=300, bbox_inches='tight')
plt.close()
print("  ✓ Saved: 08_load_vs_energy.png")

# =============================================================================
# 9. PEAK VS OFF-PEAK COMPARISON
# =============================================================================
print("\n[9/10] Creating peak vs off-peak comparison...")
...
# Users
axes[1, 1].boxplot([offpeak_data['Num_users'], peak_data['Num_users']],
                    labels=['Off-Peak', 'Peak'], patch_artist=True,
                    boxprops=dict(facecolor='#9b59b6', alpha=0.7))
axes[1, 1].set_title('Number of Users: Peak vs Off-Peak', fontsize=11, fontweight='bold')
axes[1, 1].set_ylabel('Number of Users')
axes[1, 1].grid(True, alpha=0.3, axis='y')

plt.tight_layout()
plt.savefig(f'{PLOTS_DIR}/09_peak_vs_offpeak.png', dpi=300, bbox_inches='tight')
plt.close()
print("  ✓ Saved: 09_peak_vs_offpeak.png")

# =============================================================================
# 10. SUMMARY STATISTICS
# =============================================================================
print("\n[10/10] Creating summary statistics visualization...")
...
summary_data = {
    'Metric': ['Avg Energy (W)', 'Avg PRB Usage (%)', 'Avg Traffic (MB)',
               'Avg Users', 'Avg Energy Efficiency'],
...
ax1.set_title('Key Performance Indicators - 4G Network', fontsize=14, fontweight='bold')
...
- Total Records: {len(df):,} (sampled from ~4.6M)
...
plt.savefig(f'{PLOTS_DIR}/10_summary_statistics.png', dpi=300, bbox_inches='tight')
plt.close()
...
# =============================================================================
print("\n" + "="*80)
print("4G EDA VISUALIZATION COMPLETE")
print("="*80)
print(f"\n✓ Generated 10 visualization files in '{PLOTS_DIR}/' directory")
...
print("  8. Load vs energy analysis")
print("  9. Peak vs off-peak comparison")
