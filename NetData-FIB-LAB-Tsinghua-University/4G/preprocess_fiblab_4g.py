"""
Phase 1: Data Preprocessing for FIB LAB 4G Network Dataset
===========================================================
This script performs data acquisition, inspection, cleaning, and feature engineering for 4G data.
"""
...
print("="*80)
print("FIB LAB 4G NETWORK - DATA PREPROCESSING")
print("="*80)

# =============================================================================
# 1. DATA LOADING
# =============================================================================
print("\n[1/6] Loading 4G data files...")

datasets = {}
files = {
    'Weekday': 'raw_data/Performance_4G_Weekday.txt',
    'Weekend': 'raw_data/Performance_4G_Weekend.txt'
}
...
for key, df in datasets.items():
    ...
    # Add metadata
    df_clean['network_type'] = '4G'
    df_clean['day_type'] = key

    # 4G datasets don't have energy saving columns - add them as zeros
    df_clean['Channel_shutdown_time'] = 0.0
    df_clean['Carrier_shutdown_time'] = 0.0
    df_clean['Deep_sleep_time'] = 0.0
    ...
    if (df_clean['PRB_usage_ratio'] < 0).any() or (df_clean['PRB_usage_ratio'] > 100).any():
        print(f"      Warning: Found invalid PRB usage values")
    if (df_clean['BBU_energy'] < 0).any() or (df_clean['RRU_energy'] < 0).any():
        print(f"      Warning: Found negative energy values")
    ...
    numeric_cols = ['PRB_usage_ratio', 'Traffic_volume', 'Num_users', 'BBU_energy', 'RRU_energy']
...
for key, df in cleaned_datasets.items():
    filename = f"processed_data/4g_{key.lower()}_processed.csv"
    df.to_csv(filename, index=False)
    print(f"  ✓ Saved: {filename} ({df.shape[0]} rows)")

# Combine both weekday and weekend
print("\n  Combining all 4G datasets...")
df_4g_all = pd.concat(cleaned_datasets.values(), ignore_index=True)
print(f"  ✓ Combined 4G dataset: {df_4g_all.shape[0]} rows, {df_4g_all.shape[1]} columns")
...
df_4g_all.to_csv('processed_data/fiblab_4g_all_processed.csv', index=False)
print(f"  ✓ Saved: processed_data/fiblab_4g_all_processed.csv ({df_4g_all.shape[0]} rows)")

# =============================================================================
# SUMMARY REPORT
# =============================================================================
print("\n" + "="*80)
print("4G PREPROCESSING SUMMARY")
print("="*80)

print("\nDataset Breakdown:")
print(f"  - Weekday: {cleaned_datasets['Weekday'].shape[0]} rows")
print(f"  - Weekend: {cleaned_datasets['Weekend'].shape[0]} rows")
print(f"  - Total: {df_4g_all.shape[0]} rows")

print(f"\nColumns: {df_4g_all.shape[1]}")
print(f"\nData Quality:")
print(f"  - Missing values: {df_4g_all.isnull().sum().sum()}")
print(f"  - Duplicate rows: {df_4g_all.duplicated().sum()}")
print(f"  - Unique base stations: {df_4g_all['BaseStationID'].nunique()}")
print(f"  - Unique cells: {df_4g_all['CellID'].nunique()}")

print("\nKey Statistics:")
print(df_4g_all[['PRB_usage_ratio', 'Traffic_volume', 'Num_users',
                 'BBU_energy', 'RRU_energy', 'Total_energy']].describe())

print("\nEnergy Efficiency Metrics:")
print(df_4g_all[['traffic_per_prb', 'traffic_per_user',
                 'energy_per_user', 'energy_efficiency']].describe())

print("\n" + "="*80)
print("✓ 4G Preprocessing complete!")
print("="*80)
